...
const SocialResponsibility = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#e8f5e9] to-white">
      ...
      {/* Social Programs Grid Section with Modal and Enhanced Design */}
      <section className="py-20 bg-[#0A3622]">
        <div className="container mx-auto px-4 max-w-6xl relative">
          <h2 className="text-4xl font-montserrat font-semibold text-[#FFFFFF] mb-12 text-center">Our Social Programs</h2>
          {(() => {
            ...
            const programs = [
              ...
              },
              {
                ...
                details: 'Our food distribution drives ensure that fresh, chemical-free produce reaches those who need it most. We partner with local organizations to educate communities on nutrition and promote food equity, making a tangible impact on public health and well-being.',
                ...
                statistics: [
                  ...
                  { value: '300+', label: 'Families Benefited' },
                  { value: '10+', label: 'Distribution Drives' },
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                {programs.map((program, idx) => (
                  <div
                    key={program.title}
                    className={
                      `bg-white rounded-2xl shadow-lg p-8 flex flex-col items-start text-left transition-transform duration-300 hover:scale-105 hover:shadow-2xl`}
                    style={{ overflow: 'hidden' }}
                  >
                    <LazyImage src={program.images[0]} alt={program.title} className="rounded-xl w-full h-48 object-cover mb-6" />
                    <h3 className="text-2xl font-montserrat font-semibold text-[#0A3622] mb-2">{program.title}</h3>
                    <p className="text-[#0A3622]/90 mb-4 font-montserrat">{program.description}</p>
                    <ul className="list-disc pl-5 space-y-1 text-[#0A3622] font-montserrat mb-4">
                      {program.points.map((pt, i) => <li key={i}>{pt}</li>)}
                    </ul>
                    <button onClick={() => handleOpenModal(idx)} className="mt-auto px-6 py-2 bg-[#0A3622] text-white rounded-md font-montserrat font-semibold hover:bg-[#07300f] transition-colors">Know More</button>
                  </div>
                ))}
              </div>
              {/* Modal */}
